...
const { sendMailData } = require("../utils/notification");
...
exports.createTicket = async(req, res) => {
    ...
    //put the assigne engineer on the ticket which you found from database.
    ...
    try {
        ...
        if(ticket) {
            ...
            await sendMailData("ticket is created", JSON.stringify(ticket), [user.email, engineer.email])

            res.status(201).send(objectConvertor.ticketResponse(ticket));
        }
    }catch(err) {
        console.log("Somr error happened while creating the ticket", err);
        res.status(500).send({
...
exports.updateTicket = async(req, res) => {
    ...
    if(ticket.reporter == req.userId || ticket.assignee == req.userId || savedUser.userType == constants.userTypes.admin){
        ...
        await sendMailData("ticket is updated", JSON.stringify(ticket), [reporter.email, engineer.email])
